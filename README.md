...
- Ledger recording placement: `recordTransaction` is called directly inside `deposit()` and `withdraw()` in `AccountService`. Since `transfer()` internally delegates to these two methods, both sides of a transfer are logged automatically without any transfer-specific ledger logic.
- Dependency wiring: `TransactionService` is passed into `AccountService` via constructor injection (manual, not Spring-managed) to keep `AccountService` unit-testable in isolation.